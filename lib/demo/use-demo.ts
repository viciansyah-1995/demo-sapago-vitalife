...
const send=useCallback((id:string,value:string)=>{const text=value.trim().slice(0,2000);if(!text||locks.current.has(id)||!stateRef.current[id])return false;locks.current.add(id);const result=responseFor(text,stateRef.current[id].handover);const time=new Date().toLocaleTimeString('id-ID',{hour:'2-digit',minute:'2-digit'}).replace('.',':');const key=crypto.randomUUID();const conversation=conversations.find(c=>c.id===id);if(conversation&&conversation.channel!=='website'){const channel=conversation.channel==='tiktok'?'tiktok-shop':conversation.channel==='instagram'?'instagram-dm':'shopee';void fetch(`/api/custom-channel/${channel}/inbound`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({external_user_id:id,external_message_id:key,external_conversation_id:id,text,metadata:{source:conversation.channel},sent_at:new Date().toISOString()})}).catch(()=>{});}
